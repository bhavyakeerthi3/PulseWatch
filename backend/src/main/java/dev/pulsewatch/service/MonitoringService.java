...
public class MonitoringService {
 ...
 private void evaluate(MonitoredService s,HealthCheck h){String issue=h.getStatus()==HealthCheck.State.DOWN?"Service is DOWN":h.getStatus()==HealthCheck.State.DEGRADED?"Response time above "+latencyThreshold+" ms":null;if(issue==null){alerts.findFirstByServiceIdAndMessageAndStatusInOrderByCreatedAtDesc(s.getId(),"Service is DOWN",List.of(Alert.Lifecycle.OPEN,Alert.Lifecycle.ACKNOWLEDGED)).ifPresent(a->{a.setStatus(Alert.Lifecycle.RESOLVED);events.publish("ALERT_RESOLVED",a);});return;}
  if(!alerts.existsByServiceIdAndMessageAndStatusIn(s.getId(),issue,List.of(Alert.Lifecycle.OPEN,Alert.Lifecycle.ACKNOWLEDGED))){var a=alerts.save(new Alert(s,issue,h.getStatus()==HealthCheck.State.DOWN?Alert.Severity.CRITICAL:Alert.Severity.WARNING));events.publish("ALERT_CREATED",a);if(h.getStatus()==HealthCheck.State.DOWN&&h.getFailureCount()>=2&&Duration.between(h.getCheckedAt().minusSeconds(30L*h.getFailureCount()),Instant.now()).toMinutes()>=incidentMinutes){var incident=incidents.save(new Incident(s,a));events.publish("INCIDENT_CREATED",incident);}}
 }
